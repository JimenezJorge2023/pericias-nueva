...
async function buscar() {
  ...
  try {
    ...
    tbody.innerHTML = filtrados.map(r => `
      <tr>
        <td>${r[8] || ""}</td>    // Columna I: PERICIAS RELACIONADAS
        <td>${r[17] || ""}</td>   // Columna R: PERITO ASIGNADO
        <td>${r[21] || ""}</td>   // Columna V: ESTADO
        <td>${r[19] || ""}</td>   // Columna T: SUMILLA
        <td>${r[20] || ""}</td>   // Columna U: DOCUMENTO SALIDA
      </tr>
    `).join("");
...
// Ejecutar resumen al cargar la página
