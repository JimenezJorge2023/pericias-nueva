...
async function buscar() {
  ...
  try {
    ...
    tbody.innerHTML = filtrados.map(r => `
      <tr>
        <td>${r[8] || ""}</td>   <!-- PERICIAS (columna I, índice 8) -->
        <td>${r[17] || ""}</td>  <!-- PERITO (columna R, índice 17) -->
        <td>${r[21] || ""}</td>  <!-- ESTADO (columna V, índice 21) -->
        <td>${r[19] || ""}</td>  <!-- SUMILLA (columna T, índice 19) -->
        <td>${r[20] || ""}</td>  <!-- DOCUMENTO SALIDA (columna U, índice 20) -->
      </tr>
    `).join("");
